...
from app.knowledge.models import KnowledgeCreate, KnowledgeUpdate, KnowledgeResponse, TreeNode, FileContent
...
async def list_knowledge_api(current_user: dict = Depends(get_current_user)):
...
async def get_knowledge_api(knowledge_id: str, current_user: dict = Depends(get_current_user)):
...
async def delete_knowledge_api(knowledge_id: str, current_user: dict = Depends(get_current_user)):
...
async def get_tree_api(knowledge_id: str, path: str = "", current_user: dict = Depends(get_current_user)):
    return get_file_tree(knowledge_id, path)


@router.get("/{knowledge_id}/file/{file_path:path}", response_model=FileContent)
async def get_file_api(knowledge_id: str, file_path: str, current_user: dict = Depends(get_current_user)):
...
async def create_dir_api(
    ...
    if not create_directory(knowledge_id, dir_path):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="目录已存在")
    return {"message": "创建成功"}
