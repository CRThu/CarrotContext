...
from app.git.models import GitCommitRequest, GitRevertRequest, GitCommitResponse
...
async def init_git_api(knowledge_id: str, current_user: dict = Depends(get_current_user)):
...
async def get_log_api(knowledge_id: str, limit: int = 10, current_user: dict = Depends(get_current_user)):
...
@router.post("/{knowledge_id}/commit", response_model=GitCommitResponse)
async def commit_api(
    knowledge_id: str,
    request: GitCommitRequest,
    current_user: dict = Depends(get_current_user),
):
    result = create_git_commit(knowledge_id, request.message, request.file_path)
    if not result:
